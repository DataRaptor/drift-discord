...
export const SocialsComponent = () => {
  ...
  const getSignatureFromLocalStorage = (): string => {
    var lastSignature: string | null = localStorage.getItem('lastSignature')
  ...
  useEffect(() => {
    ...
    const executeRefetchSignature = async () => {
      if (connected && publicKey) {
        ...
        if (!lastSignature) {
          lastSignature = await signAndPutSignatureinLocalStorage()
        }
        if (
  ...
  const executePostDiscordUser = async () => {
    const accessToken: string | string[] | undefined = router.query.access_token
